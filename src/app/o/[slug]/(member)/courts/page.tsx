...
export async function generateMetadata({ params }: CourtsPageProps) {
  const { slug } = await params;
  const org = await getOrgBySlug(slug);
  return {
    title: `Courts - ${org.name}`,
    description: `Browse and book pickleball courts at ${org.name}`,
  };
}
      ...
      <div className="gradient-blue-bg text-white px-4 sm:px-6 lg:px-8 pt-8 pb-12">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-white/90 text-base">
            Choose a court and book your slot. All courts are available {org.booking_window_days} days in advance.
          </p>
        </div>
      </div>
