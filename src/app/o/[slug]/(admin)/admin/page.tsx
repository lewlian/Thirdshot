...
async function getDashboardStats(organizationId: string, timezone: string) {
  ...
  const [
    ...
    // Total bookings for this org
    supabase
      .from("bookings")
      .select("*", { count: "exact", head: true })
      .eq("organization_id", organizationId),
    // Active courts for this org
    supabase
      .from("courts")
      .select("*", { count: "exact", head: true })
      .eq("organization_id", organizationId),
    // Today's confirmed bookings by checking their slots
    supabase
      .from("booking_slots")
    ...
    // Pending payment bookings for this org
    supabase
      .from("bookings")
      .select("*", { count: "exact", head: true })
      .eq("organization_id", organizationId)
